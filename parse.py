...
def parse(name, edges, triangles, cs, s, c, zbuffer, A, LightPoints, PointColors, Ka, Kd, Ks, x):
    f = open(name)
    lines = f.readlines()
    i = 0
    while i < len(lines):
        if lines[i] == "pop\n":
            ...
            k = lines[i+1].split()
            PointColors[int(k[0])] = [int(k[1]), int(k[2]), int(k[3])]
        elif lines[i] == "line\n":
            k = lines[i+1].split()
            ...
            draw_3D_triangles(triangles, s, zbuffer, A, LightPoints, PointColors, Ka, Kd, Ks, x)
            triangles = [[], [], [], []]
            i += 1
        elif lines[i] == "sphere\n":
            k = lines[i+1].split()
            add_sphere(triangles, float(k[0]), float(k[1]), float(k[2]), float(k[3]), 0.01)
            triangles = matrix_multiply(cs[-1], triangles)
            draw_3D_triangles(triangles, s, zbuffer, A, LightPoints, PointColors, Ka, Kd, Ks, x)
            triangles = [[], [], [], []]
            i += 1
            ...
            k = lines[i+1].split()
            add_torus(triangles, float(k[0]), float(k[1]), float(k[2]), float(k[3]), float(k[4]), 0.01)
            triangles = matrix_multiply(cs[-1], triangles)
            draw_3D_triangles(triangles, s, zbuffer, A, LightPoints, PointColors, Ka, Kd, Ks, x)
            triangles = [[], [], [], []]
            i += 1
